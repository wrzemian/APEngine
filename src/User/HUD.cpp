//
// Created by Misza on 01.06.2023.
//
#include "../include/User/HUD.h"
#include "stb_image.h"

void HUD::initImage(const char *path) {
...
void HUD::updateText(const std::string& newText)
{
    text = newText;
}

void HUD::renderImage(float x)
{
    // Implementation for rendering image
    shader.use();
...
void HUD::initText(const std::string& fontPath)
{
    glEnable(GL_BLEND);
...
void HUD::renderText(const std::string& text, GLfloat x, GLfloat y, GLfloat scale, const glm::vec3& color)
{
...
HUD::HUD(Shader shader, Shader textShader)
        : shader(shader), textShader(textShader)
{
}
...
GLuint HUD::compileShader(const char* source, GLenum shaderType)
{
